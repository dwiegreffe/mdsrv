...
import { createTopologyEntry, ensureTopologyDataDirectory, getTopologyEntry, listTopologyEntries, normalizePdbFileName, normalizeTopologyId, readTopologyFile, removeTopologyEntry } from './topology';
...
app.use(bodyParser.text({ type: ['chemical/x-pdb', 'text/plain'], limit: Config.upload_limit }));
...
app.get(mapPath(`${ApiRoot}/:id`), (req, res) => {
    ...
    try {
        ...
        return writeError(res, 400, e instanceof Error ? e.message : 'Invalid topology id.', 'valid-id');
    }
    const entry = getTopologyEntry(Config, id);
    if (!entry) return writeError(res, 404, `Topology '${id}' does not exist.`, 'missing-topology');
    const content = readTopologyFile(Config, id);
    if (content === void 0) return writeError(res, 404, `Topology '${id}' does not exist.`, 'missing-topology');
...
app.post(mapPath(ApiRoot), async (req, res) => {
    ...
    if (!url) return writeError(res, 400, 'Topology registration requires a source URL.', 'missing-url');
    ...
    try {
        ...
        return writeError(res, 400, e instanceof Error ? e.message : 'Invalid topology payload.', 'valid-topology');
    }

    try {
        const response = await fetch(url);
        if (!response.ok) return writeError(res, 400, `Failed to fetch topology from '${url}'.`, 'fetch-topology');
        const content = await response.text();
        const entry = createTopologyEntry(Config, { id, fileName, name, description, source, format: 'pdb' }, content);
        res.status(201);
        res.json(entry);
    } catch (e) {
        const message = e instanceof Error ? e.message : 'Failed to create topology entry.';
        const status = /already exists/i.test(message) ? 409 : 500;
        writeError(res, status, message, status === 409 ? 'unique-topology' : 'create-topology');
    }
});
...
app.put(mapPath(`${ApiRoot}/:id`), (req, res) => {
    ...
    try {
        id = normalizeTopologyId(req.params.id || '');
        fileName = normalizePdbFileName((req.query.fileName as string | undefined) || `${id}.pdb`);
    } catch (e) {
        return writeError(res, 400, e instanceof Error ? e.message : 'Invalid topology upload.', 'valid-topology');
    }

    if (typeof req.body !== 'string' || !req.body.trim()) return writeError(res, 400, 'Request body must contain PDB text.', 'request-body');

    try {
        const entry = createTopologyEntry(Config, {
            id,
            fileName,
        ...
        const message = e instanceof Error ? e.message : 'Failed to store topology upload.';
        const status = /already exists/i.test(message) ? 409 : 500;
        writeError(res, status, message, status === 409 ? 'unique-topology' : 'create-topology');
    }
});
...
app.delete(mapPath(`${ApiRoot}/:id`), (req, res) => {
    ...
    if (!removeTopologyEntry(Config, id)) return writeError(res, 404, `Topology '${id}' does not exist.`, 'missing-topology');
    res.status(200);
    res.end();
});
