...
const MagicInts = new Uint32Array([
  0, 0, 0, 0, 0, 0, 0, 0, 0, 8, 10, 12, 16, 20, 25, 32, 40, 50, 64, 80, 101,
  128, 161, 203, 256, 322, 406, 512, 645, 812, 1024, 1290, 1625, 2048, 2580,
  3250, 4096, 5060, 6501, 8192, 10321, 13003, 16384, 20642, 26007, 32768, 41285,
  52015, 65536, 82570, 104031, 131072, 165140, 208063, 262144, 330280, 416127,
  524287, 660561, 832255, 1048576, 1321122, 1664510, 2097152, 2642245, 3329021,
...
async function computeFrameStarts(path: string) {
  ...
  return new Promise<number[]>((resolve, reject) => {
    ...
    stream.on("data", (chunk: Buffer) => {
      const data: Uint8Array = concatAll([
        restData,
        new Uint8Array(chunk, chunk.byteOffset, chunk.byteLength),
      ]);
...
export async function getSingleFrameData(
  path: string,
  start: number,
  end: number,
) {
...
export async function getFrameRangeData(
  path: string,
  start: number,
  end: number,
) {
...
export async function getFrameRangeByIndexData(
  path: string,
  index: number,
  count: number,
) {
  ...
  if (index < 0 || index >= starts.length)
    throw new Error("Frame range index out of bounds.");
  if (count <= 0) throw new Error("Frame range count must be greater than 0.");

  const start = starts[index];
  const endIndex = index + count;
  const end = endIndex >= starts.length ? Infinity : starts[endIndex];
  return getFrameRangeData(path, start, end);
}
...
export function getFrameRangeFile(data: Uint8Array) {
  ...
  while (offset < dv.byteLength) {
    ...
    const box = [
      dv.getFloat32(offset + 16),
      dv.getFloat32(offset + 20),
      dv.getFloat32(offset + 24),
      dv.getFloat32(offset + 28),
      dv.getFloat32(offset + 32),
      dv.getFloat32(offset + 36),
      dv.getFloat32(offset + 40),
      dv.getFloat32(offset + 44),
      dv.getFloat32(offset + 48),
    ];
    boxes.push(box);
    ...
    if (natoms <= 9) {
      for (let i = 0; i < natoms; ++i) {
        frame.x[i] = dv.getFloat32(offset);
        offset += 4;
        frame.y[i] = dv.getFloat32(offset);
        offset += 4;
        frame.z[i] = dv.getFloat32(offset);
        offset += 4;
      }
    } else {
      const lsize = dv.getInt32(offset);
      ...
      minMaxInt[0] = dv.getInt32(offset);
      offset += 4;
      minMaxInt[1] = dv.getInt32(offset);
      offset += 4;
      minMaxInt[2] = dv.getInt32(offset);
      offset += 4;
      minMaxInt[3] = dv.getInt32(offset);
      offset += 4;
      minMaxInt[4] = dv.getInt32(offset);
      offset += 4;
      minMaxInt[5] = dv.getInt32(offset);
      offset += 4;
      let smallidx = dv.getInt32(offset);
      offset += 4;
      ...
      while (i < natoms) {
        if (bitsize === 0)
          Decoder.decodeInts(data, offset, bitsizeint[0], sizeint, thiscoord);
        else {
        ...
        if (flag === 1) {
          ...
          for (let k = 0; k < run; k += 3) {
            ...
            Decoder.decodeInts(
              data,
              offset,
              Decoder.sizeOfInts(3, sizesmall),
              sizesmall,
              thiscoord,
            );
