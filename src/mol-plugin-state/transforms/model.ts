...
import { Coordinates, Model, Queries, QueryContext, Structure, StructureElement, StructureQuery, StructureSelection as Sel, Topology, ArrayTrajectory, Trajectory, XtcTrajectoryStream } from '../../mol-model/structure';
...
export { TrajectoryXTCFromModelAndServer };
...
async function getXTCTrajectoryStream(ctx: RuntimeContext, plugin: PluginContext, obj: StateObject, url: string) {
    if (obj.type === SO.Molecule.Topology.type) {
        try {
            ...
            return new XtcTrajectoryStream(plugin, url, startOffsets, model);
        } catch (e) {
            console.log(e);
            plugin.log.error(e);
        }
...
type TrajectoryXTCFromModelAndServer = typeof TrajectoryXTCFromModelAndServer
const TrajectoryXTCFromModelAndServer = PluginStateTransform.BuiltIn({
    name: 'trajectory-stream-from-model-and-server',
    display: { name: 'Trajectory Stream from Topology & Server', description: 'Create a trajectory from existing model/topology and trajectory on server.' },
    from: SO.Root,
    to: SO.Molecule.Trajectory,
    params: {
        modelRef: PD.Text('', { isHidden: true }),
        trajectoryUrl: PD.Text('', { isHidden: true }),
    }
})({
    apply({ params, dependencies }, plugin: PluginContext) {
        return Task.create('Create trajectory stream from model/topology and trajectory on server', async ctx => {
            const trajectory = await getXTCTrajectoryStream(ctx, plugin, dependencies![params.modelRef], params.trajectoryUrl);
            const props = { label: 'Trajectory', description: `${trajectory.frameCount} model${trajectory.frameCount === 1 ? '' : 's'}` };
            return new SO.Molecule.Trajectory(trajectory, props);
        });
    }
