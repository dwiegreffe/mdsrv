...
const SimpleSettingsMapping = ParamMapping({
    params: (ctx: PluginUIContext) => {
        ...
        if (controls) {
            ...
            if (controls.extension !== 'none') options.push(['extension', LayoutOptions.extension]);
    ...
    target(ctx: PluginUIContext) {
        ...
        if (r.extension !== 'hidden' && (!c || c.extension !== 'none')) layout.push('extension');
    ...
    async apply(props, ctx) {
        ...
        const state = produce(ctx.layout.state, s => {
            ...
            s.regionState.extension = props.layout.indexOf('extension') >= 0 ? 'full' : 'hidden';
