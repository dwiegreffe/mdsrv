...
{ params: PD.Values<RemoteStateSnapshots['Params']>, entries: OrderedMap<string, RemoteEntry>, isBusy: boolean }> {
    ...
    render() {
        return <>
            ...
            <RemoteStateSnapshotList entries={this.state.entries} isBusy={this.state.isBusy} serverUrl={this.state.params.options.serverUrl} open={'snapshot-url'}
...
export class RemoteStateSnapshotList extends PurePluginUIComponent<
{
    entries: OrderedMap<string, RemoteEntry>,
    serverUrl: string,
    open: string,
    isBusy: boolean,
    fetch: (e: React.MouseEvent<HTMLElement>) => void,
    ...
    open = async (e: React.MouseEvent<HTMLElement>) => {
        ...
        window.open(`${url}?${this.props.open}=${encodeURIComponent(entry.url)}`, '_blank');
    };
